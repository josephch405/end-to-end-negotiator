...
import data
from engines.rnn_variational_engine import RnnVariationalEngine
from domain import get_domain
from models.utils import *
from models.ctx_encoder import MlpContextEncoder


class RnnVariationalModel(nn.Module):
    corpus_ty = data.WordCorpus
    engine_ty = RnnVariationalEngine
    ...
    def __init__(self, word_dict, item_dict, context_dict, count_dict, args):
        ...
        # Encoder GRU outputs
        self.reader = nn.GRU(
            args.nhid_ctx + args.nembed_word, args.nhid_lang * 2, bias=True)
    ...
    def read(self, inpt, lang_h, ctx_h, prefix_token='THEM:'):
        ...
        ctx_h_rep = ctx_h.expand(inpt_emb.size(
            0), ctx_h.size(1), ctx_h.size(2))
    ...
    def generate_choice_logits(self, inpt, lang_h, ctx_h):
        ...
        logits = [decoder.forward(h).squeeze(0)
    ...
    def write_batch(self, bsz, lang_h, ctx_h, temperature, max_words=100):
        ...
        for _ in range(max_words):
            ...
            scores.sub_(scores.max(1)[0].expand(
                scores.size(0), scores.size(1)))
    ...
    def sample(self, mu, std):
        return mu + std * torch.randn_like(std).cuda()
    ...
    def forward_lm(self, inpt_emb, lang_h, ctx_h):
        ...
        muAndStd, _ = self.reader(inpt_emb, lang_h)
        # Handle sampling
        mu = muAndStd[:, :, :self.args.nhid_lang]
        std = muAndStd[:, :, self.args.nhid_lang:]
    ...
    def forward_selection(self, inpt_emb, lang_h, ctx_h):
        ...
        logit = self.attn(h.view(-1, 2 * self.args.nhid_attn)
                          ).view(h.size(0), h.size(1))
        prob = F.softmax(logit, dim=1).unsqueeze(2).expand_as(h)
        attn = torch.sum(torch.mul(h, prob), 1,
    ...
    def forward(self, inpt, ctx):
        ...
        lang_h = self.zero_h(ctx_h.size(1), self.args.nhid_lang * 2)

        inpt_emb = self.word_encoder(inpt)
        inpt_emb = self.word_encoder_dropout(inpt_emb)

        out, lang_hs, kl_div = self.forward_lm(inpt_emb, lang_h, ctx_h)
        sel_out = self.forward_selection(inpt_emb, lang_hs, ctx_h)

        return out, sel_out, kl_div
